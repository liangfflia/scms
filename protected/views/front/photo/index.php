...
<div class="gallery">
			<?$photosArr = SArray::slice(4, $photos)?>
    
			<?foreach($photosArr as $obj):?>
<ul class="thumbnails">
				<?foreach($obj as $val):?>
				    <li class="span3" align="center">
					    <a class="thumbnail fancybox-button zoomer" data-rel="fancybox-button" title="<?=$val->title?> <a class='btn btn-small' href='#'><i class='icon-star'></i> </a>" href="/<?=$val->src?>">
						...
						<div align="center">
							<a class="btn btn-small <?=in_array($val->id, $favedIds)?'faved':'fav'?>" data-id="<?=$val->id?>" href="#"><i class="icon-star"></i> </a>
						</div>
				</li>
			<?endforeach;?>
			    </ul>
...
<?//$this->endWidget();?>
		...
		<div class="span3">
			...
			<div class="headline photoCats">
				<h3 style="margin-bottom: 20px; margin-left:10px;">Категории</h3>
					<div style="margin-left: 15px; line-height: 30px;"><a class="<?=( !isset($cat->id))?'style-color':''?>" href="<?=Yii::app()->baseUrl.'/photo'?>">Все</a></div>
				<?foreach($categories as $val):?>
					<div style="margin-left: 15px; line-height: 30px;"><a class="<?=( isset($cat->title) && ($val->title == $cat->title))?'style-color':''?>" href="<?=Yii::app()->baseUrl.'/photo/'.$val->id?>"><?=$val->title?></a></div>
				<?endforeach;?>
			</div>
